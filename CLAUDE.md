...
**One URL, one process, one port: `http://127.0.0.1:3210`.** In dev the server embeds Vite in middleware mode (`packages/server/src/dev-web.ts`, activated by `OPENACME_WEB_DEV=<path to apps/web>` which `packages/server/scripts/dev.mjs` sets) — `/api/*` goes to Hono, everything else to Vite, and the HMR WebSocket shares the same server. There is no separate web dev process or derived web port. Two parallel `pnpm dev` sessions against different data dirs just work as long as their `server.port`s differ:
